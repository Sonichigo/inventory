...
-- Two tables, no indexes, join on LOWER() columns = expensive nested loop + seq scans
-- At 60 req/sec this creates serious DB wait time visible in DBMarlin.
...
-- BAD: no indexes on join columns — forces sequential scans on both tables
...
-- Bulk insert 50,000 random inventory rows
...
-- Bulk insert 50,000 random supplier rows
